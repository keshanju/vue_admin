...
import { CardOtherDefineApi, CardOtherAbandonApi } from '@/api/CardOtherApi';
/**
 * 第三方充值卡
 */
@Component({
...
export default class ListController extends BaseVue {
    ...
    */
    private async getDataListPager() {
        // 数据源
        let ds = this.getDataGridPager(async (strWhere, pageSize, pageIndex) => await new CardOtherAbandonApi().getListPager(this.options.id, strWhere, pageSize, pageIndex));
